...
function extractCompetitiveRankImg (options, p) {
  return normalizeValue(options, p('.masthead-player .competitive-rank .img[src]').text())
}
...
export function scrapePlayerStats (options, html) {
  ...
  stats.competitiveRankImg = extractCompetitiveRankImg(options, p)
